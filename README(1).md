...
This repository contains the synthetic test dataset used to evaluate the KQL detection queries and validate the threat hunting model developed in the master's thesis *"Threat Hunting Model for ClickFix Technique in Enterprise Environments"*.
...
Use `clickfix_model_validation_manifest.csv` to verify the 14 model validation scenarios from Section 4.6.2. Each scenario is identified by its `ModelScenarioId` (MV-01 through MV-14) and linked to its dataset entries via `ReportIds`.
...
Use `clickfix_artifact_recovery_manifest.csv` to verify the artifact recovery rates reported in Section 4.6.2. Each row maps a `ReportId` to its `ArtifactType` for a specific model scenario — allowing independent verification of which artifact types were recoverable per scenario.
...
The dataset passed a full automated audit across all 7 files:

- ✅ 181 log entries across 4 tables — no duplicates, no cross-table contamination
- ✅ All required MDE schema fields present and populated in each table
- ✅ No Scenario, Classification, or VariationCategory fields in table files — raw MDE format only
- ✅ All 181 ReportIds covered by the KQL test manifest with correct classifications
- ✅ All 14 model validation scenarios verified — ReportIds match correct dataset scenarios
- ✅ All 30 artifact recovery entries verified — ArtifactType matches table for every ReportId
- ✅ All 9 endpoint-touching ClickFix variations covered by attack scenarios
- ✅ All 100 benign scenarios unique
- ✅ All timestamps within March 2026
- ✅ No attack command patterns present in benign entries

---

## Generation

Generated using Python with a fixed random seed (`seed=42`) for full reproducibility. All timestamps are in March 2026. Command-line arguments in attack entries are sourced from the threat intelligence reports cited in the thesis. The dataset was generated by Claude Sonnet 4.6 (claude-sonnet-4-6) as part of the thesis research process.

---

## Citation

> *Threat Hunting Model for ClickFix Technique in Enterprise Environments*, Master's Thesis, 2026.

---

## Threat Intelligence Sources

- Proofpoint Threat Research — [*From Clipboard to Compromise: A PowerShell Self-Pwn*](https://www.proofpoint.com/us/blog/threat-insight/clipboard-compromise-powershell-self-pwn) (2024)
- Proofpoint Threat Research — [*Around the World in 90 Days: State-Sponsored Actors Try ClickFix*](https://www.proofpoint.com/us/blog/threat-insight/around-world-90-days-state-sponsored-actors-try-clickfix) (2025)
- Microsoft Threat Intelligence — [*Think Before You Click(Fix): Analyzing the ClickFix Social Engineering Technique*](https://www.microsoft.com/en-us/security/blog/2025/08/21/think-before-you-clickfix-analyzing-the-clickfix-social-engineering-technique/) (2025)
- Sekoia TDR — [*From Contagious to ClickFake Interview: Lazarus Leveraging the ClickFix Tactic*](https://blog.sekoia.io/clickfake-interview-campaign-by-lazarus/) (2025)
- Elastic Security Labs — [*A Wretch Client: From ClickFix Deception to Information Stealer Deployment*](https://www.elastic.co/security-labs/a-wretch-client) (2025)
- Microsoft Threat Intelligence — [*New ClickFix Variant CrashFix Deploying Python RAT Trojan*](https://www.microsoft.com/en-us/security/blog/2026/02/05/clickfix-variant-crashfix-deploying-python-rat-trojan/) (2026)
- SOCRadar — [*ClickFix & FileFix: How a Copy-Paste Trick Became 2025's Top Social Engineering Threat*](https://socradar.io/blog/clickfix-filefix-copy-paste-top-social-engineering/) (2025)
- Push Security — [*ConsentFix: Browser-Native ClickFix Hijacks OAuth Grants*](https://pushsecurity.com/blog/consentfix) (2025)
- The Hacker News — [*New FileFix Variant Delivers StealC Malware Through Multilingual Phishing Site*](https://thehackernews.com/2025/09/new-filefix-variant-delivers-stealc.html) (2025)
- Microsoft Threat Intelligence — [*Phishing Campaign Impersonates Booking.com, Delivers Credential-Stealing Malware*](https://www.microsoft.com/en-us/security/blog/2025/03/13/phishing-campaign-impersonates-booking-com-delivers-a-suite-of-credential-stealing-malware/) (2025)
